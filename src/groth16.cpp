...
{
#if defined(USE_CUDA)
    void multiMulByScalarGpu(AltBn128::Engine::G1Point &r, AltBn128::Engine::G1PointAffine *bases, uint8_t *scalars, unsigned int sizePoint, unsigned int n)
    {
        LOG_DEBUG("use gpu msm");
...
#include <blst_t.hpp>
    ...
    std::unique_ptr<Proof<Engine>> Prover<Engine>::prove(typename Engine::FrElement *wtns)
    {
...
#if defined(USE_CUDA)
        multiMulByScalarGpu(pi_a, pointsA, (uint8_t *)wtns, sizeof(affine_t), nVars);
#else
...
#if defined(USE_CUDA)
        multiMulByScalarGpu(pib1, pointsB1, (uint8_t *)wtns, sizeof(affine_t), nVars);
#else
...
#if defined(USE_CUDA)
        multiMulByScalarGpu(pi_c, pointsC, (uint8_t *)((uint64_t)wtns + (nPublic + 1) * sW), sizeof(affine_t), nVars - nPublic - 1);
#else
        ...
        auto pB2_future = std::async(
            ...
            {
#if defined(USE_CUDA)
                multiMulByScalarGpu(pi_b, pointsB2, (uint8_t *)wtns, sizeof(affine_t), nVars);
#else
                E.g2.multiMulByScalar(pi_b, pointsB2, (uint8_t *)wtns, sW, nVars);
#endif
            });
...
#if defined(USE_CUDA)
        multiMulByScalarGpu(pih, pointsH, (uint8_t *)a, sizeof(affine_t), domainSize);
#else
